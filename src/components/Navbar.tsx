    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <a href="/#" className="flex items-center gap-2 group">
            <div
              className={cn(
                ...
                // Sem fundo branco (como pedido). Mantemos só um contorno sutil para legibilidade no Hero.
                isScrolled ? "ring-1 ring-border shadow-sm" : "ring-1 ring-white/30"
              )}
            >
            ...
            <span
              className={cn(
                "font-display font-bold text-2xl md:text-3xl tracking-tight transition-colors",
        ...
        <div className="hidden md:flex items-center gap-4">
          <Button variant="ghost" size="icon" className={cn("rounded-full", isScrolled ? "text-foreground" : "text-white hover:bg-white/20 hover:text-white")}>
          ...
          >
            <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer">Assinar Agora</a>
          </Button>
        </div>

        {/* Mobile Menu Button */}
        <button
          className={cn("md:hidden p-2", isScrolled ? "text-foreground" : "text-foreground md:text-white")}
